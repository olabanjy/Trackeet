...
def export_track_xls(request):
    ...
    rows = Track.objects.all().values_list( 'display_upc', 'volume_no', 'track_no', 'hidden_track', 'title','song_version', 'genre', 'isrc', 'track_duration', 'preview_clip_start_time', 'preview_clip_duration', 'p_line', 'recording_artist', 'artist', 'release_name', 'parental_advisory','label_name', 'producer', 'publisher', 'writer', 'arranger', 'territories','exclusive','wholesale_price', 'download','sales_start_date','sales_end_date','error','processed_day')
    for row in rows:
        row_num += 1
...
def export_album_xls(request):
    ...
    columns = ['Product UPC', 'Total Volumes', 'Total Tracks', 'Product Title', 'Product Artist', 'Genre', 'CLINE', 'Release Date', 'Imprint Label',  'Artist URL', 'Catalog Number', 'Manufacturer UPC', 'Deleted?', 'Territories Carveouts', 'Master Carveouts','Processed Day',]
    ...
    rows = Album.objects.all().values_list('product_upc', 'total_volume', 'total_tracks', 'product_title', 'product_artist', 'genre', 'cline', 'release_date', 'imprint_label', 'artist_url', 'catalog_number', 'manufacture_upc', 'deleted', 'territories_carveouts','master_carveouts', 'processed_day' )
    for row in rows:
        row_num += 1
        for col_num in range(len(row)):
            ws.write(row_num, col_num, row[col_num], font_style)
...
def export_album_csv(request):
    ...
    writer.writerow(['Product UPC', 'Total Volumes', 'Total Tracks', 'Product Title', 'Product Artist', 'Genre', 'CLINE', 'Release Date', 'Imprint Label',  'Artist URL', 'Catalog Number', 'Manufacturer UPC', 'Deleted?', 'Territories Carveouts', 'Master Carveouts','Processed Day',])
...
def export_drmsys_xls(request):
    ...
    columns = [ 'Arranger', 'Artist', 'ArtistURL', 'BPM', 'C_Line', 'DataType', 'Deleted?', 'DisplayUPC', 'Download', 'Error', 'Exclusive', 'Genre', 'GenreAlt', 'GenreSub', 'GenreSubAlt', 'HiddenTrack', 'ISRC', 'ItunesLink', 'LabelCatalogNo', 'LabelName', 'Language', 'ManufacturerUPC', 'MasterCarveouts', 'P_Line', 'ParentalAdvisory', 'PreviewClipDuration', 'PreviewClipStartTime', 'PriceBand', 'Producer', 'Publisher', 'RecordingArtist', 'ReleaseDate', 'ReleaseName', 'RoyaltyRate', 'SalesEndDate', 'SalesStartDate', 'SongVersion', 'Territories', 'TerritoriesCarveouts', 'Title', 'TotalTracks', 'TotalVolumes', 'TrackDuration', 'TrackNo', 'VendorName', 'VolumeNo', 'WholesalePrice', 'Writer', 'zIDKey_UPCISRC', 'Processed Day', ]
    ...
    rows = Master_DRMSYS.objects.all().values_list('arranger', 'artist', 'artist_url', 'bpm', 'c_line', 'data_type', 'deleted', 'display_upc', 'download', 'error', 'exclusive', 'genre', 'genre_alt', 'genre_sub', 'genre_subalt', 'hidden_track', 'isrc', 'itunes_link', 'label_catalogno', 'label_name', 'language', 'manufacturer_upc', 'master_carveouts', 'p_line', 'parental_advisory', 'preview_clip_duration', 'preview_clip_start_time', 'price_band', 'producer', 'publisher', 'recording_artist', 'release_date', 'release_name', 'royalty_rate', 'sales_end_date', 'sales_start_date', 'song_version', 'territories', 'territories_carveouts', 'title', 'total_tracks', 'total_volumes', 'track_duration', 'track_no', 'vendor_name', 'volume_no', 'wholesale_price', 'writer', 'zIDKey_UPCISRC', 'processed_day')
    for row in rows:
        row_num += 1
        for col_num in range(len(row)):
            ws.write(row_num, col_num, row[col_num], font_style)
...
def export_drmsys_csv(request):
    ...
    writer.writerow([ 'Arranger', 'Artist', 'ArtistURL', 'BPM', 'C_Line', 'DataType', 'Deleted?', 'DisplayUPC', 'Download', 'Error', 'Exclusive', 'Genre', 'GenreAlt', 'GenreSub', 'GenreSubAlt', 'HiddenTrack', 'ISRC', 'ItunesLink', 'LabelCatalogNo', 'LabelName', 'Language', 'ManufacturerUPC', 'MasterCarveouts', 'P_Line', 'ParentalAdvisory', 'PreviewClipDuration', 'PreviewClipStartTime', 'PriceBand', 'Producer', 'Publisher', 'RecordingArtist', 'ReleaseDate', 'ReleaseName', 'RoyaltyRate', 'SalesEndDate', 'SalesStartDate', 'SongVersion', 'Territories', 'TerritoriesCarveouts', 'Title', 'TotalTracks', 'TotalVolumes', 'TrackDuration', 'TrackNo', 'VendorName', 'VolumeNo', 'WholesalePrice', 'Writer', 'zIDKey_UPCISRC', 'Processed Day', ])
        ...
        try:
            if form.is_valid():
                ...
                if check_album:
                    ...
                    new_album = Album(product_title = product_title, product_upc = product_upc, genre = genre, release_date = release_date, total_volume = total_volume, imprint_label = imprint_label, product_artist = product_artist, artist_url =artist_url, catalog_number = catalog_number, manufacture_upc = manufacture_upc, deleted = deleted, total_tracks = total_tracks, cline = cline, territories_carveouts = territories_carveouts, master_carveouts = master_carveouts, processed_day = today)
                    new_album.save()
...
class VendorPPL(View):
    def get(self, *args, **kwargs):
        ...
        metadata_form = UploadPPLAccountFormMetadata()
    ...
    def post(self, *args, **kwargs):
        metadata_form = UploadPPLAccountFormMetadata(self.request.POST, self.request.FILES or None)
        accounting_form = UploadPPLAccountFormAccounting(self.request.POST, self.request.FILES or None)
        try:
            # if metadata_form.is_valid():
            #     ppl_metadata_file = metadata_form.cleaned_data['ppl_metadata_file']
            #     if not ppl_metadata_file.name.endswith('.xlsx'):
            #         messages.info(self.request, "Incorrect file format, File must be a xlsx format")
            #         return HttpResponseRedirect(reverse("core:ppl"))
            #     processed_doc = ProcessedDocument.objects.filter(file_name=ppl_metadata_file.name).first()
            #     if processed_doc:
            #         messages.info(self.request, "This metadata file has been processed earlier. Please confirm !")
            #         return HttpResponseRedirect(reverse("core:ppl"))
            #     else:
            #         new_processed_doc = ProcessedDocument(file_name=ppl_metadata_file.name,file_doc = ppl_metadata_file, which_doc = "PPL")
            #         new_processed_doc.save()
            #         process_ppl(new_processed_doc.file_name)
            #         messages.info(self.request, "Metadata sent for processing, you will get a prompt after processing! ")
            #     return HttpResponseRedirect(reverse("core:ppl"))
            if accounting_form.is_valid():
                ...
                if processed_doc:
                    ...
                    new_processed_doc.save()
                    process_ppl_account.delay(new_processed_doc.pk)
                    messages.info(self.request, "Metadata sent for processing, you will get a prompt after processing! ")
                return HttpResponseRedirect(reverse("core:ppl"))
            else:  
                messages.info(self.request, "Form is not valid  !")
